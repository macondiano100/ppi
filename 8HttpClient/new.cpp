...
int main(int argc,char*argv[])
{
    ...
    cout<<"Deme una url"<<endl;
    getline(cin,url);
    }
    if(getURLInfo(url,urlInfo))
    {
        cout<<"Host: "<<urlInfo.domain<<endl;
        cout<<"Protocolo:"<<urlInfo.scheme<<endl;
        cout<<"Ruta: "<<urlInfo.path<<endl;
        cout<<"Puerto: "<<urlInfo.port<<endl;
    }
    else
    {
        cout<<"URL No valida"<<endl;
    }
    return 0;
}
bool getURLInfo(string url,URLInfo &info)
...
string portToService(int port)
{
    ...
    appl_name = getservbyport(port,"TCP");
    if (!appl_name)
        return "";
    else
    {
...
string servToPort(string service)
{
    ...
    {
        return to_string(appl_name->s_port);
    }
}
